// dnfl-exporter-v5.js v5.0
/* ==========================================================================
   DNFL Commissioner Data Exporter Engine v5.0
   Repository: rosario-jason/dnfl
   File: scripts/dnfl-exporter-v5.js
...
(function() {
    ...
    /**
     * Helper to fetch Player Database Metadata via DNFLClient with Multi-Key ID Normalization
     */
    async function getPlayersMap() {
        ...
        try {
            ...
            pArray.forEach(p => {
                ...
                // Index by raw ID, unpadded ID, and 4-digit padded ID to guarantee 100% match hits
    ...
    /**
     * REPORT 1: Rosters for all teams with YTD points & ROS projections for a given week
     */
    async function generateRostersReport(week) {
        ...
        const [rostersData, ytdScoresData, rosScoresData] = await Promise.all([
            ...
            client.fetchData('playerScores', week ? `&W=${week}` : '&W=YTD'),
            client.fetchData('projectedScores', week ? `&W=${week}` : '&W=ROS')
        ]);
        ...
        if (ytdScoresData?.playerScores?.playerScore) {
            toArray(ytdScoresData.playerScores.playerScore).forEach(ps => {
                ...
                ytdMap[unpaddedPid] = parseFloat(score).toFixed(2);
            });
        }
        ...
        if (rosScoresData?.projectedScores?.playerScore) {
            toArray(rosScoresData.projectedScores.playerScore).forEach(ps => {
                ...
                rosMap[unpaddedPid] = parseFloat(proj).toFixed(2);
            });
        }
        ...
        rosterList.forEach(f => {
            ...
            players.forEach(p => {
                ...
                rows.push({
                    ...
                    "ROS Projected Points": rosMap[pid] || rosMap[unpaddedPid] || '0.00'
                });
            });
        });
        ...
        return {
            title: `DNFL Team Rosters with YTD & ROS Projections (Week ${week || 'Current'}, ${targetYear})`,
            description: `Complete roster listing across all franchises including player YTD fantasy points and rest-of-season projections for Week ${week || 'Current'}.`,
            columns: ["Week", "Franchise ID", "Team Name", "Player ID", "Player Name", "Position", "NFL Team", "Roster Status", "YTD Points", "ROS Projected Points"],
            rows: rows
        };
    }
    ...
    async function generateWeeklyDetailsReport(week) {
        ...
        const [weeklyData, ytdScoresData, rosScoresData] = await Promise.all([
            ...
            client.fetchData('playerScores', `&W=${week}`),
            client.fetchData('projectedScores', `&W=${week}`)
        ]);
        ...
        if (ytdScoresData?.playerScores?.playerScore) {
            toArray(ytdScoresData.playerScores.playerScore).forEach(ps => {
                const pid = String(ps.id).trim();
                const unpaddedPid = pid.replace(/^0+/, '');
                const score = ps.score || ps.points || 0;
                ytdMap[pid] = parseFloat(score).toFixed(2);
                ytdMap[unpaddedPid] = parseFloat(score).toFixed(2);
            });
        }
    ...
    async function generateStandingsReport() {
        ...
        franchiseStandings.forEach((s, idx) => {
            ...
            let divRecord = '0-0-0';
            if (s.div_record) {
                divRecord = s.div_record;
            } else if (s.div && typeof s.div === 'object') {
                divRecord = `${s.div.w || 0}-${s.div.l || 0}-${s.div.t || 0}`;
            } else {
                const dw = s.divw !== undefined ? s.divw : (s.div_w !== undefined ? s.div_w : (s.dw !== undefined ? s.dw : (s.divwins !== undefined ? s.divwins : 0)));
                const dl = s.divl !== undefined ? s.divl : (s.div_l !== undefined ? s.div_l : (s.dl !== undefined ? s.dl : (s.divlosses !== undefined ? s.divlosses : 0)));
                const dt = s.divt !== undefined ? s.divt : (s.div_t !== undefined ? s.div_t : (s.dt !== undefined ? s.dt : (s.divties !== undefined ? s.divties : 0)));
                divRecord = `${dw}-${dl}-${dt}`;
            }
