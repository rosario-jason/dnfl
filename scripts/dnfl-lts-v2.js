...
(function() {
     ...
     * Handles all MFL API JSON variations (weeklyResult singular, weeklyResults plural, direct matchups, single week objects)
     * @param {Object} data 
    ...
    async function init() {
        ...
        try {
            ...
            cachedFranchises.forEach(f => {
                ...
                // If assigned confId doesn't exist in cachedConferences, default to first valid conference
    ...
    function renderLtsModuleView() {
        ...
        // Safety Fallback: If selected conference has no teams, switch to first conference with teams
        if (confTeams.length === 0 && cachedConferences.length > 0) {
            ...
            if (validConf) {
                ...
                // All teams fall back to all franchises
