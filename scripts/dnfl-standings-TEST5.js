/* ==========================================================================
   DNFL Dynamic Standings & Seeding Engine (v3.00-TEST5)
...
(function () {
    ...
    // Dynamic Execution Context Variables
    ...
    /**
     * Resolves active season year and league ID dynamically at execution time
     */
    function getActiveContext() {
        let yr = window.current_year || window.mflYear || window.year;
        if (!yr && window.DNFL && window.DNFL.Client && window.DNFL.Client.getContext) {
            const ctx = window.DNFL.Client.getContext();
            yr = ctx ? ctx.year : null;
        }
        if (!yr && window.location) {
            const urlParams = new URLSearchParams(window.location.search);
            yr = urlParams.get('YEAR');
            if (!yr && window.location.pathname) {
                const pathSegments = window.location.pathname.split('/');
                const foundYear = pathSegments.find(segment => /^20\d{2}$/.test(segment));
                if (foundYear) yr = foundYear;
            }
        }
        if (!yr) yr = new Date().getFullYear().toString();

        let lId = window.league_id || window.mflLeagueId;
        if (!lId && window.DNFL && window.DNFL.Client && window.DNFL.Client.getContext) {
            const ctx = window.DNFL.Client.getContext();
            lId = ctx ? ctx.leagueId : null;
        }
        if (!lId && window.location && window.location.search) {
            const urlParams = new URLSearchParams(window.location.search);
            lId = urlParams.get('L') || '22883';
        }
        if (!lId) lId = '22883';

        return { year: String(yr), leagueId: String(lId) };
    }
    ...
    async function init() {
        ...
        targetYear = activeCtx.year;
        leagueId = activeCtx.leagueId;
    ...
    function buildTeamRowHtml(profile, divId, confRules, rowCounter) {
        ...
        if (hasSeasonStarted) {
            const seed = teamSeeds[normFid] || "-";
            let badgeIcons = '';

            const isDivWinner = profile.division && normFid === divLeaders[norm(profile.division)];
        ...
        const activeFranchiseId = getLoggedInFranchiseId();
        const myTeamClass = (activeFranchiseId && normFid === activeFranchiseId) ? " dnfl-my-team" : "";
