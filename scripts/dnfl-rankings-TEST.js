...
(function() {
    ...
    function renderChartAndTable(records, activeConfFilter = 'All') {
        ...
        const userFranchiseId = normId(
            window.mfl_franchise_id || 
            window.franchise_id || 
            (window.DNFLClient && typeof window.DNFLClient.getFranchiseId === 'function' ? window.DNFLClient.getFranchiseId() : '')
        );

        sortedData.forEach((item, index) => {
            const row = document.createElement('tr');
            if (userFranchiseId && normId(item.franchiseId) === userFranchiseId) {
                row.classList.add('dnfl-myfranchise');
            }
