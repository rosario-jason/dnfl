/* ==========================================================================
   DNFL Dynamic Standings & Seeding Engine (v3.00-TEST9)
   Duke Networking Fantasy League (DNFL)
   Domain: https://dnfl.live
   ========================================================================== */
...
(function () {
    ...
    /**
     * Inlined Default Rules Configuration
     * Guarantees 0ms synchronous boot without requiring external HTTP fetches or triggering CORB.
     */
    let STANDINGS_RULES = {
        "2006-2024": {
        ...
        "default": {
            ...
            "conferenceOverrides": {
                ...
                "02": {
                    "seedingModel": "mfl_native",
                    "playoffCutoff": 4,
                    "hasDivisionCrown": false,
                    "relegation": { "enabled": false }
                }
            }
        }
    };
    ...
    /**
     * Normalizes 2-digit Conference and Division IDs.
     * Safely returns '' for null, undefined, or empty values to prevent false matches.
     */
    function norm(val) {
    ...
    /**
     * Ensures target input is returned as an Array.
     */
     ...
     * Evaluates URL parameter -> URL path -> MFL global variables -> API Middleware context.
     */
    function getActiveContext() {
        ...
        // Keep global window context in sync so DNFL.Client fetches data for the correct season
        if (!window.mflYear) window.mflYear = resolvedYear;
        if (!window.current_year) window.current_year = resolvedYear;
    ...
    function setupDropdown(targetYear) {
        ...
        // Explicitly bind event listeners for dropdown changes
    ...
    function buildTeamRowHtml(profile, divId, confRules, rowIdx, targetYear) {
        ...
        const logoUrl = profile.icon || "https://dnfl.live/images/ficon-dnfl.png";
            ...
            <tr class="${rowClass} ${myTeamClass} ${divRowClass}">
                ...
                <td>
                    <div style="display: flex; align-items: center; gap: 12px; text-align: left;">
                        <img src="${logoUrl}" alt="${teamName}" class="franchiseicon" style="width: 32px; height: 32px; border-radius: 50%; object-fit: cover;" onerror="this.src='https://dnfl.live/images/ficon-dnfl.png'">
                        <div>
    ...
    async function init() {
        ...
        try {
            ...
            if (!apiClient) {
                throw new Error("[DNFL.Standings] DNFL API middleware unavailable.");
            }

            // Fetch data feeds via API client middleware
            const [standingsResponse, leagueResponse, rawRulesJson] = await Promise.all([
                ...
                apiClient.fetchRawText(rulesUrl).catch(err => {
                    // Non-blocking catch: falls back cleanly to inlined STANDINGS_RULES
                    return null;
                })
            ]);

            if (!standingsResponse || !leagueResponse) {
                throw new Error("[DNFL.Standings] Missing structural configuration maps from MFL payload.");
            }

            if (rawRulesJson) {
                try {
                    const fetchedRules = JSON.parse(rawRulesJson);
                    if (fetchedRules && typeof fetchedRules === 'object') {
                        STANDINGS_RULES = { ...STANDINGS_RULES, ...fetchedRules };
                    }
                } catch (e) {
                    console.warn("[DNFL.Standings] Could not parse remote rules JSON. Using inlined rules.", e);
                }
            }
    ...
    /**
     * Event-driven, Observer-backed Auto Initialization
     */
    function autoInit() {
        if (window.DNFL && window.DNFL.Utils && window.DNFL.Utils.onElementReady) {
            window.DNFL.Utils.onElementReady('#dnfl-standings-tbody', init);
        } else {
            if (document.getElementById('dnfl-standings-tbody')) {
                init();
            } else {
