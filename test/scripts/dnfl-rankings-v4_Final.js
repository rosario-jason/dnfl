...
(function() {
    ...
    /**
     * Helper to fetch raw CSV with dual fallback using API middleware
     */
    ...
    function renderChartAndTable(records, activeConfFilter = 'All') {
        ...
        const titleEl = document.querySelector('#dnfl-rankings-container .dnfl-card-title') || document.getElementById("dnfl-rankings-title");
        if (titleEl) {
            if (isFiltered) {
    ...
    function toggleElementVisibility(sectionType) {
        if (sectionType === 'chart') {
            ...
            const isNowHidden = wrapper.classList.toggle('dnfl-is-hidden');
            if (!isNowHidden) {
                button.innerHTML = '<i class="fa-solid fa-chart-bar"></i> HIDE CHART';
                applyConferenceFilter();
            } else {
                button.innerHTML = '<i class="fa-solid fa-chart-bar"></i> SHOW CHART';
            }
        }
    }
