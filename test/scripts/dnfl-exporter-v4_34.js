/* ==========================================================================
   DNFL Commissioner Data Exporter Engine v4.34 (Architecture Aligned)
   Duke Networking Fantasy League (DNFL)
   Aligned with dnfl-standings-v4_09.js & dnfl-api-client-v3_36.js architecture.
   ...
   Refactored to eliminate inline style assignments in favor of framework state class .dnfl-is-hidden.
   ========================================================================== */
(function() {
    'use strict';
    ...
    const WEEK1_BASELINE_SCORES = {
        '0001': 84.6, '0002': 76.6, '0003': 76.6, '0004': 72.6, '0005': 68.7,
        '0006': 75.9, '0007': 71.4, '0008': 72.6, '0009': 76.5, '0010': 71.3,
        '0011': 65.5, '0012': 74.6, '0013': 68.6, '0014': 70.9, '0015': 78.9,
        '0016': 71.7, '0017': 84.3, '0018': 93.6, '0019': 79.4, '0020': 66.1,
        '0021': 78.8, '0022': 72.1, '0023': 72.5, '0024': 78.0, '0025': 70.4,
        '0026': 63.3, '0027': 74.9, '0028': 78.6, '0029': 77.1, '0030': 75.2,
        '0031': 64.8, '0032': 76.3, '0033': 72.1, '0034': 87.2, '0035': 68.0,
        '0036': 67.0
    };
    ...
    async function generatePowerRankingsReport(targetWeek) {
        ...
        const weekNum = (targetWeek !== undefined && targetWeek !== null && targetWeek !== "") ? parseInt(targetWeek, 10) : 1;
        ...
        for (let w = Math.max(1, weekNum); w <= totalRegWeeks; w++) {
        ...
        // Step 1: Calculate Composite Projection Score for each team
        statList.forEach(s => {
            s.compProj = (s.starterRos * 0.80) + (s.benchRos * 0.20);
            
            const totalGames = s.wins + s.losses + s.ties;
            s.h2hPctVal = totalGames > 0 ? (s.wins + 0.5 * s.ties) / totalGames : 0;

            const totalAllPlay = s.allPlayWins + s.allPlayLosses + s.allPlayTies;
            s.allPlayPctVal = totalAllPlay > 0 ? (s.allPlayWins + 0.5 * s.allPlayTies) / totalAllPlay : 0;
        });

        // Step 2: Determine Dynamic Weekly Overarching Weights (Dynamic Regular Season Length)
        let regWeeks = 12;
        if (cachedLeague && cachedLeague.lastRegularSeasonWeek) {
            regWeeks = parseInt(cachedLeague.lastRegularSeasonWeek, 10) || 12;
        } else if (window.DNFL && window.DNFL.leagueMetadata && window.DNFL.leagueMetadata.lastRegularSeasonWeek) {
            regWeeks = parseInt(window.DNFL.leagueMetadata.lastRegularSeasonWeek, 10) || 12;
        }

        let perfWeight = 0.0;
        let projWeight = 1.0;
        const currentWeekClamped = Math.max(0, Math.min(regWeeks, weekNum));
        if (weekNum > 0) {
            const stepDenominator = Math.max(1, regWeeks - 1);
            perfWeight = 0.20 + (0.65 * (currentWeekClamped - 1) / stepDenominator);
            projWeight = 1.0 - perfWeight;
        }

        // Step 3: Sub-allocate Performance Bucket Weights
        const allPlayWeight = perfWeight * 0.50;
        const totalPointsWeight = perfWeight * 0.30;
        const h2hWeight = perfWeight * 0.20;

        // Step 4 & 5: Standardize Metrics via Population Z-Scores & Apply Linear Transformation (Center 80, Factor 6.5)
        function calcZScaledScores(values) {
        ...
        const pfValues = statList.map(s => s.pf);
        const h2hValues = statList.map(s => s.h2hPctVal);
        const allPlayValues = statList.map(s => s.allPlayPctVal);
        const projValues = statList.map(s => s.compProj);

        const scaledPfScores = calcZScaledScores(pfValues);
        const scaledH2hScores = calcZScaledScores(h2hValues);
        const scaledAllPlayScores = calcZScaledScores(allPlayValues);
        const scaledProjScores = calcZScaledScores(projValues);

        // Step 6: Aggregate Final Power Score
        statList.forEach((s, idx) => {
            s.scaledTotalPoints = scaledPfScores[idx];
            s.scaledH2h = scaledH2hScores[idx];
            s.scaledAllPlay = scaledAllPlayScores[idx];
            s.scaledProj = scaledProjScores[idx];

            if (weekNum === 0) {
                s.perfScoreA = 80.0;
            } else {
                s.perfScoreA = (s.scaledAllPlay * 0.50) + (s.scaledTotalPoints * 0.30) + (s.scaledH2h * 0.20);
            }
            s.rosScoreB = s.scaledProj;

            const finalPowerScoreRaw = (s.scaledAllPlay * allPlayWeight) +
                                       (s.scaledTotalPoints * totalPointsWeight) +
                                       (s.scaledH2h * h2hWeight) +
                                       (s.scaledProj * projWeight);

            s.pureAutoIndex = parseFloat(finalPowerScoreRaw.toFixed(1));

            // Dynamic Early-Season Baseline Ramp (Isolated for 2026 Legacy Transition)
            let rampFactor = 1.0;
            const is2026Season = String(targetYear) === '2026';

            if (is2026Season) {
                if (weekNum === 1) {
                    rampFactor = 0.0; // 100% Week 1 Manual Baseline
                } else if (weekNum === 2) {
                    rampFactor = 0.33; // 33% Automated, 67% Week 1 Baseline
                } else if (weekNum === 3) {
                    rampFactor = 0.66; // 66% Automated, 34% Week 1 Baseline
                }
            }

            const baselineScore = WEEK1_BASELINE_SCORES[s.fid] !== undefined ? WEEK1_BASELINE_SCORES[s.fid] : s.pureAutoIndex;
            const rampedScore = (s.pureAutoIndex * rampFactor) + (baselineScore * (1.0 - rampFactor));

            s.calculatedIndex = parseFloat(rampedScore.toFixed(1));
        });
        ...
        return {
            ...
            description: `Z-Score Standardized 60-100 Scale Power Index: Performance Weight ${(perfWeight * 100).toFixed(1)}% (All-Play 50%, PF 30%, H2H 20%) + Projection Weight ${(projWeight * 100).toFixed(1)}% (80% Starter / 20% Bench ROS) at Week ${currentWeekClamped} of ${totalRegWeeks}. Centered at 80 with 6.5 dispersion.`,
            columns: ["Rank", "Franchise ID", "Team Name", "Owner", "Conference", "Points For (PF)", "H2H Record", "H2H Win %", "All-Play Record", "All-Play Win %", "Perf Score (A)", "Starter ROS Proj", "Bench ROS Proj", "ROS Score (B)", "Power Index", "Tier", "Owner Roast / Commentary"],
            rows: rows
        };
    }
