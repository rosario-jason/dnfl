/* ==========================================================================
   DNFL Commissioner Data Exporter Engine v4.16 (Architecture Aligned)
...
(function() {
    ...
    async function getLeagueInfo() {
        ...
        try {
            const [leagueData, statusData] = await Promise.all([
                ...
                client.fetchData('mfl_status').catch(() => ({}))
            ]);
            cachedLeague = leagueData?.league || {};
            cachedFranchises = toArray(cachedLeague?.franchises?.franchise);
            cachedMflStatus = statusData?.mfl_status || statusData?.status || {};

            const envWk = (typeof window !== 'undefined' && (window.current_week || window.mflCurrentWk || window.mfl_current_week)) 
                || cachedMflStatus?.currentWk || cachedMflStatus?.current_week || cachedMflStatus?.week 
                || cachedLeague?.currentWk || cachedLeague?.current_week;
            if (envWk) {
                cachedLeague.currentWk = String(envWk);
            }
        } catch (err) {
    ...
    function resolveCurrentWeek() {
        const rawWk = (cachedLeague && (cachedLeague.currentWk || cachedLeague.current_week))
            || (cachedMflStatus && (cachedMflStatus.currentWk || cachedMflStatus.current_week || cachedMflStatus.week))
    ...
    function populateWeekDropdown() {
        ...
        if (parsedTargetYear >= currentYearNum) {
            // Active Season: Range from Week 1 up to current week, capped at lastRegularSeasonWeek
            if (detectedWk !== null) {
                maxWeek = Math.min(detectedWk, lastRegWk);
            } else {
                maxWeek = lastRegWk; // Safety fallback before API returns
            }
        } else {
            // Historical Season: Show all regular season weeks (1 to lastRegularSeasonWeek)
            maxWeek = lastRegWk;
        }
        ...
        if (prevVal && parseInt(prevVal, 10) <= maxWeek) {
