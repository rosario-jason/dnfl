/* ==========================================================================
   DNFL Power Rankings Dashboard Engine v4.01 (Architecture Aligned)
...
(function() {
            ...
            if (isPreseasonWeek) {
                thead.innerHTML = `
                    <tr>
                        ...
                        <th class="dnfl-col-roast">Commentary</th>
                    </tr>
                `;
            } else {
                thead.innerHTML = `
        ...
        sortedData.forEach((item, index) => {
            ...
            const franchiseColHtml = `
                <div style="display: flex; align-items: center; gap: 10px; text-align: left;">
                    <img src="${item.iconUrl}" alt="${item.franchiseName}" class="franchiseicon" style="width: 32px; height: 32px; border-radius: 50%; object-fit: cover;" onError="this.onerror=null;this.src='https://dnfl.live/images/ficon-dnfl.png';" />
                    <div style="display: flex; flex-direction: column;">
                        <span class="dnfl-team-name" style="font-weight: 700; color: var(--dnfl-text-heading, #0f172a); font-size: 0.9rem;">${item.franchiseName}</span>
                        <span class="dnfl-owner-name" style="font-size: 0.75rem; color: var(--dnfl-text-subtle, #64748b);">${item.ownerName}</span>
                    </div>
                </div>
            `;
            ...
            const hasRoast = Boolean(item.comments && item.comments.trim().length > 0);
            const roastBtnHtml = hasRoast ? `
                <button class="dnfl-btn dnfl-btn-secondary" style="padding: 4px 8px; font-size: 0.75rem;" onclick="DNFL.Rankings.toggleCommentRow('${item.franchiseId}')">
                    <i class="fa-solid fa-comment-dots"></i> Roast
                </button>
            ...
            if (isPreseasonWeek) {
                mainRow.innerHTML = `
                    ...
                    <td class="dnfl-col-index" style="font-weight: 700;">${item.powerIndex.toFixed(1)}</td>
                    <td class="dnfl-col-record dnfl-hide-mobile">${recordVal}</td>
                    <td class="dnfl-col-roast">${roastBtnHtml}</td>
                `;
            } else {
                mainRow.innerHTML = `
                    <td class="dnfl-col-rank">${rankBadgeHtml}</td>
            ...
            if (hasRoast) {
                ...
                subRow.id = `dnfl-roast-row-${item.franchiseId}`;
                subRow.style.display = 'none';
                subRow.style.backgroundColor = 'var(--dnfl-bg-muted, #f8fafc)';

                const colSpanCount = isPreseasonWeek ? 5 : 7;
                subRow.innerHTML = `
                    <td colspan="${colSpanCount}" style="padding: 12px 16px; border-left: 3px solid var(--dnfl-primary, #0577B1); text-align: left;">
                        <div style="font-size: 0.85rem; color: var(--dnfl-text-main, #121212); font-style: italic; line-height: 1.4;">
                            <i class="fa-solid fa-fire-flame-curved" style="color: #ef4444; margin-right: 6px;"></i>
                            <strong>Commissioner's Roast:</strong> ${item.comments}
                        </div>
                    </td>
                `;
    ...
    function toggleCommentRow(franchiseId) {
        const subRow = document.getElementById(`dnfl-roast-row-${franchiseId}`);
        if (!subRow) return;
