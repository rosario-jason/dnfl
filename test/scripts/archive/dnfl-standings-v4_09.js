/* ==========================================================================
   DNFL Dynamic Standings & Seeding Engine v4.09 (Architecture Aligned)
...
(function() {
    ...
    async function init() {
        ...
        try {
            ...
            tbody.innerHTML = `<tr><td colspan="6" class="dnfl-status-error">Error loading standings.</td></tr>`;
        }
    }
    ...
    function renderStandingsKey(confRules) {
        ...
        if (confRules.hasDivisionCrown) {
            iconHtml += `<span class="dnfl-legend-item"><i class="fa-solid fa-crown dnfl-icon-blue"></i> Div Winner</span>`;
        }
        if (confRules.playoffCutoff) {
            iconHtml += `<span class="dnfl-legend-item"><i class="fa-solid fa-trophy dnfl-icon-amber"></i> Playoffs</span>`;
        }
        if (confRules.promotion?.enabled) {
            iconHtml += `<span class="dnfl-legend-item"><i class="fa-solid fa-circle-arrow-up dnfl-icon-green"></i> Promotion</span>`;
        }
        if (confRules.relegation?.enabled) {
            iconHtml += `<span class="dnfl-legend-item"><i class="fa-solid fa-circle-arrow-down dnfl-icon-red"></i> Relegation</span>`;
        }
        iconHtml += `</div>`;
    ...
    function buildTeamRowHtml(profile, divId, confRules, rowCounter) {
        ...
        let seedCellContent = `<span class="dnfl-seed-badge">-</span>`;
        if (hasSeasonStarted) {
            ...
            if (confRules.hasDivisionCrown && isDivWinner) {
                badgeIcons += `<i class="fa-solid fa-crown dnfl-icon-blue" title="Division Winner"></i>`;
            }
            if (seed !== "-" && confRules.playoffCutoff && seed <= confRules.playoffCutoff) {
                badgeIcons += `<i class="fa-solid fa-trophy dnfl-icon-amber" title="Playoff Seed #${seed}"></i>`;
            }
            if (relegatedTeamIds.has(profile.id)) {
                badgeIcons += `<i class="fa-solid fa-circle-arrow-down dnfl-icon-red" title="Relegation Zone"></i>`;
            }
            if (promotedTeamIds.has(profile.id)) {
                badgeIcons += `<i class="fa-solid fa-circle-arrow-up dnfl-icon-green" title="Promotion Zone"></i>`;
            }

            seedCellContent = `<span class="dnfl-seed-badge">${seed}</span>${badgeIcons}`;
        }
    ...
    function toggleDnflDivision(divId) {
        ...
        const currentlyHidden = rows[0].classList.contains('dnfl-is-hidden');

        rows.forEach(row => {
            row.classList.toggle('dnfl-is-hidden', !currentlyHidden);
        });
