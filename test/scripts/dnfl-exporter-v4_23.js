/* ==========================================================================
   DNFL Commissioner Data Exporter Engine v4.23 (Architecture Aligned)
...
(function() {
    ...
    function formatPlayerName(rawName) {
        ...
        try {
            ...
            if (window.DNFL && window.DNFL.leagueMetadata && window.DNFL.leagueMetadata.currentWk) {
                cachedLeague = window.DNFL.leagueMetadata;
            } else {
                const data = await client.fetchData('league');
                cachedLeague = data?.league || {};
                if (window.DNFL && window.DNFL.leagueMetadata) {
                    cachedLeague = { ...window.DNFL.leagueMetadata, ...cachedLeague };
                }
            }
    ...
    function populateWeekDropdown() {
        ...
        if (parsedTargetYear >= currentYearNum) {
            ...
            if (detectedWk !== null) {
                ...
                maxWeek = 1; // Default to Week 1 until API resolves active week
            }
        } else {
