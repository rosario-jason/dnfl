...
(function() {
    ...
    async function generatePowerRankingsReport(targetWeek) {
        ...
        // Calculate Component A (Performance), Component B (ROS), and Component C (Power Index) on 60-100 scale
        statList.forEach(s => {
            ...
            const pfRatio = maxPf > 0 ? (s.pf / maxPf) : 0;

            // Component A Index (0 to 1.0) -> Scaled to 60-100
            const compA = (pfRatio * 0.40) + (allPlayPct * 0.40) + (h2hPct * 0.20);
            s.perfScoreA = parseFloat((60 + (compA * 40)).toFixed(1));

            // Component B Index (0 to 1.0) -> Scaled to 60-100
            const compB = maxRos > 0 ? (s.weightedRos / maxRos) : 0;
            s.rosScoreB = parseFloat((60 + (compB * 40)).toFixed(1));

            // Component C (Power Index) = Simple Weighted Average of Score A and Score B
            s.calculatedIndex = parseFloat(((weightA * s.perfScoreA) + (weightB * s.rosScoreB)).toFixed(1));
        ...
        return {
            ...
            description: `Power Index (60-100 Scale): ${(weightA * 100).toFixed(1)}% Perf Score A (60-100) + ${(weightB * 100).toFixed(1)}% ROS Score B (60-100) over ${totalRegWeeks} regular season weeks. Starter 80% / Bench 20% weighting. Includes rankings_overrides.json support.`,
