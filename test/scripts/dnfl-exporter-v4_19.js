/* ==========================================================================
   DNFL Commissioner Data Exporter Engine v4.19 (Architecture Aligned)
    ...
    async function getLeagueInfo() {
        ...
        try {
            const [leagueData, standingsData] = await Promise.all([
                ...
                client.fetchData('leagueStandings').catch(() => ({}))
            ]);
            cachedLeague = leagueData?.league || {};
            cachedFranchises = toArray(cachedLeague?.franchises?.franchise);

            const curWk = standingsData?.leagueStandings?.currentWk 
                || leagueData?.league?.currentWk
