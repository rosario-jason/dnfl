/* ==========================================================================
   DNFL Dynamic Standings & Seeding Engine v4.02 (v4 Architecture Aligned)
   Duke Networking Fantasy League (DNFL)
   Fully aligned with dnfl-global-v3_36.css & _test_v4_36.scss design tokens.
   ========================================================================== */
(function() {
    'use strict';
    ...
    function renderStandingsKey(confRules) {
        ...
        if (confRules.hasDivisionCrown) {
            iconHtml += `<span class="dnfl-key-item"><i class="fa-solid fa-crown" style="color: var(--dnfl-badge-blue);"></i> Div Winner</span>`;
        }
        if (confRules.playoffCutoff) {
            iconHtml += `<span class="dnfl-key-item"><i class="fa-solid fa-trophy" style="color: var(--dnfl-badge-amber);"></i> Playoffs</span>`;
        }
        if (confRules.promotion?.enabled) {
            iconHtml += `<span class="dnfl-key-item"><i class="fa-solid fa-circle-arrow-up" style="color: var(--dnfl-success-green);"></i> Promotion</span>`;
        }
        if (confRules.relegation?.enabled) {
            iconHtml += `<span class="dnfl-key-item"><i class="fa-solid fa-circle-arrow-down" style="color: var(--dnfl-alert-red);"></i> Relegation</span>`;
        }
        iconHtml += `</div>`;
    ...
    function getTableSubheaderHtml() {
        return `
    ...
    function buildTeamRowHtml(profile, divId, confRules, rowCounter) {
        ...
        const pfBadgeHtml = `<span class="dnfl-badge dnfl-badge-green">${pf}</span>`;
        const paBadgeHtml = `<span class="dnfl-badge dnfl-badge-red">${pa}</span>`;
        ...
        return `
            <tr class="${rowClass}">
                <td class="dnfl-col-seed" style="white-space: nowrap;">
                    ${seedCellContent}
                </td>
                ...
                <td class="dnfl-col-record" style="text-align: center;">
    ...
    function updateDnflStandingsView() {
        ...
        const tbody = document.getElementById("dnfl-standings-tbody");
        const titleSpan = document.getElementById("dnfl-standings-title");
        ...
        if (selectedValue === 'playoffs') {
            if (titleSpan) titleSpan.textContent = "DNFL Playoff Standings";
            renderStandingsKey(globalRules);

            tableHtml += getTableSubheaderHtml();
        ...
        if (titleSpan) titleSpan.textContent = `${conf.name} Standings`;
        ...
        divisionsToRender.forEach(div => {
            ...
            let formattedDivTitle = div.name;
            if (hasRealDivision && conf.name) {
                if (!div.name.toLowerCase().startsWith(conf.name.toLowerCase())) {
                    formattedDivTitle = `${conf.name} ${div.name}`;
                }
            }
